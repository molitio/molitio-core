import React from 'react';
import { createUseStyles } from 'react-jss';
import { SvgComponentProps } from '../../common/interface/SvgComponentProps';

export const LogoSenseRadioSvg: React.FC<SvgComponentProps> = ({ ...props }) => {
    const style = createUseStyles({
        svg: {
            /* position: props.dimensions === 'FULLSCREEN' ? 'fixed' : 'absolute', */
            height: props.dimensions === 'FULLSCREEN' ? '100vh' : props.dimensions.height,
            width: props.dimensions === 'FULLSCREEN' ? '100vw' : props.dimensions.width,
            pointerEvents: 'none',
        },
        st0: {
            fillRule: 'evenodd',
            clipRule: 'evenodd',
            fill: '#FFFFFF',
        },
        st1: {
            fillRule: 'evenodd',
            clipRule: 'evenodd',
            fill: '#2A2A2A',
        },
        st2: {
            fillRule: 'evenodd',
            clipRule: 'evenodd',
            fill: '#FFC82F',
        },
    }).apply({});

    return (
        <svg id="playa" className={style.svg} version="1.1" viewBox="0 0 800 400" xmlns="http://www.w3.org/2000/svg">
            <polygon
                id="playc"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="151.6 504.6 145.2 504.6 145.2 511 145.2 515.6 145.2 522.1 145.2 532.9 151.6 532.9 151.6 522.1 173.3 522.1 173.3 515.6 151.6 515.6 151.6 511 173.3 511 173.3 504.6"
            />
            <polygon
                id="playd"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="360.7 532.9 367.1 532.9 388.8 532.9 388.8 526.5 367.1 526.5 367.1 522.1 388.8 522.1 388.8 515.6 367.1 515.6 367.1 511 388.8 511 388.8 504.6 367.1 504.6 360.7 504.6 360.7 511 360.7 515.6 360.7 522.1 360.7 526.5"
            />
            <polygon
                id="playe"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="550 511 550 504.6 528.3 504.6 521.9 504.6 521.9 511 521.9 526.5 521.9 532.9 528.3 532.9 550 532.9 550 526.5 528.3 526.5 528.3 511"
            />
            <rect
                id="playf"
                className={style.st0}
                x="728.07"
                y="346.72"
                width="9.7165"
                height="48.453"
                stroke-width="1.6151"
            />
            <polygon
                id="playg"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="484.5 511 506.3 511 506.3 504.6 484.5 504.6 478.1 504.6 478.1 511 478.1 515.6 478.1 519.8 478.1 522.1 499.9 522.1 499.9 526.5 478.1 526.5 478.1 532.9 499.9 532.9 506.3 532.9 506.3 526.5 506.3 522.1 506.3 517.3 506.3 515.6 484.5 515.6"
            />
            <polygon
                id="playh"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="451.7 504.7 445.3 504.7 445.3 526.4 445.3 532.8 451.7 532.8 467.1 532.8 473.5 532.8 473.5 526.4 473.5 504.7 467.1 504.7 467.1 526.4 451.7 526.4"
            />
            <polygon
                id="playi"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="423.7 532.8 430.1 532.8 430.1 511.1 434.5 511.1 434.5 532.8 440.9 532.8 440.9 511.1 440.9 504.7 434.5 504.7 430.1 504.7 423.7 504.7 419.1 504.7 412.7 504.7 412.7 511.1 412.7 532.8 419.1 532.8 419.1 511.1 423.7 511.1"
            />
            <polygon
                id="playj"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="355.9 515.6 355.9 504.6 349.5 504.6 349.5 515.6 334.2 515.6 334.2 504.6 327.8 504.6 327.8 515.6 327.8 522 327.8 532.9 334.2 532.9 334.2 522 349.5 522 349.5 532.9 355.9 532.9 355.9 522"
            />
            <polygon
                id="playk"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="305.9 511 305.9 532.9 312.3 532.9 312.3 511 323.2 511 323.2 504.6 312.3 504.6 305.9 504.6 295 504.6 295 511"
            />
            <polygon
                id="playl"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="271.2 532.9 271.2 526.5 249.5 526.5 249.5 504.6 243.1 504.6 243.1 526.5 243.1 532.9 249.5 532.9"
            />
            <polygon
                id="playm"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="210.3 532.9 216.7 532.9 238.4 532.9 238.4 526.5 216.7 526.5 216.7 522.1 238.4 522.1 238.4 515.6 216.7 515.6 216.7 511 238.4 511 238.4 504.6 216.7 504.6 210.3 504.6 210.3 511 210.3 515.6 210.3 522.1 210.3 526.5"
            />
            <polygon
                id="playn"
                className={style.st0}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="177.7 532.9 184.1 532.9 205.8 532.9 205.8 526.5 184.1 526.5 184.1 522.1 205.8 522.1 205.8 515.6 184.1 515.6 184.1 511 205.8 511 205.8 504.6 184.1 504.6 177.7 504.6 177.7 511 177.7 515.6 177.7 522.1 177.7 526.5"
            />
            <polygon
                id="playo"
                className={style.st1}
                transform="matrix(1.5182 0 0 1.7182 -48.189 -520.45)"
                points="414.1 385.7 531 385.7 548.6 406.7 548.6 495.1 143.1 495.1 125.5 474 125.5 415 63.5 415 45.9 394 45.9 305.6 396.6 305.6 414.1 326.7"
            />
            <path
                id="playp"
                className={style.st0}
                d="m747.5 157.72h-107.34v121.82h107.34v-27.663h-82.894v-19.072h82.894v-27.663h-82.894v-19.759h82.894zm-509.05 75.085v19.072h-82.894v27.663h107.34v-74.226h-82.894v-19.759h82.894v-27.663h-107.34v75.085zm145.14 19.072h-82.894v-19.072h82.894v-27.663h-82.894v-19.759h82.894v-27.663h-107.34v121.82h107.34zm95.647 27.319-56.629-79.209h-0.15181v79.553h-24.291v-121.82h26.417l56.933 79.553v-79.553h24.443v121.82h-26.417v-2.4055zm123.28-46.391v19.072h-82.894v27.663h107.34v-74.226h-82.894v-19.759h82.894v-27.663h-107.34v75.085z"
                stroke-width="1.6151"
            />
            <path
                id="playq"
                className={style.st2}
                d="m262.89 19.064h-107.34v121.82h24.443v-46.735h58.451v46.735h24.443v-94.157zm-145.75 27.663v19.759h-58.451v-19.759zm24.443 19.759v-47.422h-107.19v121.82h24.443v-46.735h17.763l32.793 46.735h32.186l-32.793-46.735h32.793v-11.168zm96.862 0h-58.451v-19.759h58.451zm279.35-19.759v66.494h-58.451v-66.494zm24.443 66.494v-94.157h-107.34v121.82h107.34zm-120.09-94.157h-24.443v121.82h24.443zm-59.665 0h-86.841v121.82h107.34v-94.157zm-3.9473 94.157h-58.451v-66.494h58.451z"
                stroke-width="1.6151"
            />
        </svg>
    );
};
